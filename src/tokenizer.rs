...
impl Tokenizer {
    ...
    pub fn tokenize<'s, E: Into<tokenizers::EncodeInput<'s>> + Send>(
        &self,
        input: Vec<E>,
        ...
        for encoding in encodings {
            input_ids.push_row(ndarray::ArrayView::from(
                &Self::to_i64(encoding.get_ids()).to_vec(),
            ))?;
            attn_masks.push_row(ndarray::ArrayView::from(
                &Self::to_i64(encoding.get_attention_mask()).to_vec(),
            ))?;
        }
