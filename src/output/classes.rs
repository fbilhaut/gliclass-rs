...
impl Classes {
    pub fn try_from(tensors: OutputTensors) -> Result<Self> {
        let logits = tensors
            .outputs
            .get(TENSOR_LOGITS)
            .ok_or_else(|| format!("expected tensor not found in model output: {TENSOR_LOGITS}"))?;
    ...
    pub fn scores(&self, index: usize) -> Option<Vec<f32>> {
        if index < self.scores.nrows() {
            Some(self.scores.row(index).to_vec())
        } else {
            None
        }
    }
